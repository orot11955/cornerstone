...
| [007 Migration/Release](./adr/0007-migration-release.md)           | Accepted | expand/backfill/contract, deploy와 restore                               | M3                        |
...
- `Theme`, `Style`, `Brand`, `Density`는 독립 축이며 custom Brand는 registry의 명시적 allowlist와 4.5:1 contrast 검증 뒤 CSS variable로 적용한다.
...
- test DB custom-format `pg_dump`를 새 격리 DB에 restore하고 table/user/audit/Migration checksum을 비교하는 logical backup Gate

남은 완료 조건:

- 차기 schema release부터 직전 immutable application artifact/fixture upgrade와 expand schema의 N/N-1 read/write Gate
- M5에서 password hashing·audit 계약과 함께 운영 admin protected one-time bootstrap job과 break-glass audit 구현
