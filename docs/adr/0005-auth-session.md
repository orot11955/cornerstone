...
- Release 전 Root `pnpm auth:password:benchmark`를 실행한다. 이 명령은 `apps/api/release/auth-password-policy.json`의 Argon2id parameter와 hash/verify p95 목표를 검증하고, 정책 digest·표본 수·p50/p95/max·runtime/OS/CPU 메타데이터만 `.artifacts/auth/password-benchmark.json`에 기록한다. CI의 `Auth password release benchmark` Gate는 이 명령의 성공과 evidence artifact 업로드를 강제한다. `benchmark:password:fast`는 개발 확인용 `non-production-test` evidence이므로 release 증거로 사용할 수 없다.
