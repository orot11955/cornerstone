...
const standardV6PredecessorSnapshotSchema = z
  .object({
    schemaVersion: z.literal(3),
    templateVersion: z.literal('0.6.0'),
...
export type StandardV6PredecessorSnapshot = z.infer<typeof standardV6PredecessorSnapshotSchema>

export interface ResolvedStandardV6Predecessor {
  snapshot: StandardV6PredecessorSnapshot
  fragments: StandardV6PredecessorSnapshot['fragments']
  composers: StandardV6PredecessorSnapshot['composers']
  outputs: StandardV6PredecessorSnapshot['outputs']
...
export async function loadStandardV6PredecessorSnapshot(): Promise<StandardV6PredecessorSnapshot> {
  return standardV6PredecessorSnapshotSchema.parse(
    JSON.parse(
...
export function parseStandardV6PredecessorSnapshot(value: unknown): StandardV6PredecessorSnapshot {
  return standardV6PredecessorSnapshotSchema.parse(value)
}

export async function resolveStandardV6Predecessor(
  manifest: ResolvedManifest,
  scaffoldsInput: readonly ScaffoldRegistryEntry[],
): Promise<ResolvedStandardV6Predecessor> {
  const scaffolds = validateScaffoldRegistry(scaffoldsInput)
  assertScaffoldContract(scaffolds)
  const snapshot = await loadStandardV6PredecessorSnapshot()
  ...
  if (metadata.templateVersion !== version) {
    throw new Error('Immutable Standard 0.6.0 metadata version mismatch')
  }
  const contents = new Map<string, Buffer>()
  const outputs: StandardV6PredecessorSnapshot['outputs'] = []
...
export function buildStandardV6PredecessorLock(
  resolved: ResolvedStandardV6Predecessor,
...
export async function readStandardV6AdoptionSource(path: string): Promise<Buffer> {
  const snapshot = await loadStandardV6PredecessorSnapshot()
  const source = snapshot.adoptionSources.find((candidate) => candidate.path === path)
  if (!source) throw new Error(`Standard 0.6.0 adoption source is not declared: ${path}`)
  return readSnapshotFile(`adoption-sources/${path}`, source.checksum)
}

export async function readStandardV6AdoptionTarget(path: string): Promise<Buffer> {
  const snapshot = await loadStandardV6PredecessorSnapshot()
  const target = snapshot.adoptionTargets.find((candidate) => candidate.path === path)
  if (!target) throw new Error(`Standard 0.6.0 adoption target is not declared: ${path}`)
...
function assertScaffoldContract(scaffolds: readonly ScaffoldRegistryEntry[]): void {
  for (const scaffold of scaffolds) {
    if (scaffold.version !== 1 && scaffold.version !== 2 && scaffold.version !== 3) {
      throw new Error('Standard 0.6.0 does not support this scaffold registry entry version')
    }
  }
}
...
async function renderOutput(
  ...
  if (!declared) throw new Error(`Standard 0.6.0 output is not declared: ${path}`)
  ...
  if (path === 'README.md') {
    ...
    if (!source.startsWith(heading)) throw new Error('Standard 0.6.0 README fixture is invalid')
...
function nestModule(
  ...
  if (!definition?.nestModule) throw new Error(`Immutable Standard 0.6.0 composer missing: ${id}`)
...
function composeRoutePolicies(source: string, entries: readonly ScaffoldRegistryEntry[]): string {
  ...
  if (!source.includes(marker)) {
    throw new Error('Immutable Standard 0.6.0 route policy marker is missing')
  }
  const policies = [...entries]
    .sort((left, right) => left.id.localeCompare(right.id))
    .map((entry) => {
      if (entry.kind !== 'api' || entry.version !== 3) {
        throw new Error(`Invalid Standard 0.6.0 API contribution: ${entry.id}`)
      }
...
async function readSnapshotFile(relativePath: string, checksum: string): Promise<Buffer> {
  ...
  if (sha256(content) !== checksum) {
    throw new Error(`Immutable Standard 0.6.0 snapshot checksum mismatch: ${relativePath}`)
  }
  return content
}
