...
const standardV4PredecessorSnapshotSchema = z
  .object({
    schemaVersion: z.literal(2),
    templateVersion: z.literal('0.4.0'),
...
export type StandardV4PredecessorSnapshot = z.infer<typeof standardV4PredecessorSnapshotSchema>

export interface ResolvedStandardV4Predecessor {
  snapshot: StandardV4PredecessorSnapshot
  fragments: StandardV4PredecessorSnapshot['fragments']
  composers: StandardV4PredecessorSnapshot['composers']
  outputs: StandardV4PredecessorSnapshot['outputs']
...
export async function loadStandardV4PredecessorSnapshot(): Promise<StandardV4PredecessorSnapshot> {
  return standardV4PredecessorSnapshotSchema.parse(
    JSON.parse(
...
export function parseStandardV4PredecessorSnapshot(value: unknown): StandardV4PredecessorSnapshot {
  return standardV4PredecessorSnapshotSchema.parse(value)
}

export async function resolveStandardV4Predecessor(
  manifest: ResolvedManifest,
  scaffoldsInput: readonly ScaffoldRegistryEntry[],
): Promise<ResolvedStandardV4Predecessor> {
  const scaffolds = validateScaffoldRegistry(scaffoldsInput)
  assertScaffoldContract(scaffolds)
  const snapshot = await loadStandardV4PredecessorSnapshot()
  ...
  if (metadata.templateVersion !== version) {
    throw new Error('Immutable Standard 0.4.0 metadata version mismatch')
  }
  const contents = new Map<string, Buffer>()
  const outputs: StandardV4PredecessorSnapshot['outputs'] = []
...
export function buildStandardV4PredecessorLock(
  resolved: ResolvedStandardV4Predecessor,
...
export async function readStandardV4AdoptionSource(path: string): Promise<Buffer> {
  const snapshot = await loadStandardV4PredecessorSnapshot()
  const source = snapshot.adoptionSources.find((candidate) => candidate.path === path)
  if (!source) throw new Error(`Standard 0.4.0 adoption source is not declared: ${path}`)
  return readSnapshotFile(`adoption-sources/${path}`, source.checksum)
}

export async function readStandardV4AdoptionTarget(path: string): Promise<Buffer> {
  const snapshot = await loadStandardV4PredecessorSnapshot()
  const target = snapshot.adoptionTargets.find((candidate) => candidate.path === path)
  if (!target) throw new Error(`Standard 0.4.0 adoption target is not declared: ${path}`)
...
function assertScaffoldContract(scaffolds: readonly ScaffoldRegistryEntry[]): void {
  for (const scaffold of scaffolds) {
    if (scaffold.version !== 1 && scaffold.version !== 2 && scaffold.version !== 3) {
      throw new Error('Standard 0.4.0 does not support this scaffold registry entry version')
    }
  }
}
...
async function renderOutput(
  ...
  if (!declared) throw new Error(`Standard 0.4.0 output is not declared: ${path}`)
  ...
  if (path === 'README.md') {
    ...
    if (!source.startsWith(heading)) throw new Error('Standard 0.4.0 README fixture is invalid')
...
function nestModule(
  ...
  if (!definition?.nestModule) throw new Error(`Immutable Standard 0.4.0 composer missing: ${id}`)
...
function composeRoutePolicies(source: string, entries: readonly ScaffoldRegistryEntry[]): string {
  ...
  if (!source.includes(marker)) {
    throw new Error('Immutable Standard 0.4.0 route policy marker is missing')
  }
  const policies = [...entries]
    .sort((left, right) => left.id.localeCompare(right.id))
    .map((entry) => {
      if (entry.kind !== 'api' || entry.version !== 3) {
        throw new Error(`Invalid Standard 0.4.0 API contribution: ${entry.id}`)
      }
...
async function readSnapshotFile(relativePath: string, checksum: string): Promise<Buffer> {
  ...
  if (sha256(content) !== checksum) {
    throw new Error(`Immutable Standard 0.4.0 snapshot checksum mismatch: ${relativePath}`)
  }
  return content
}
